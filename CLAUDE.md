...
- **`vercel.ts` sets `framework: "nextjs"`** (typed config, replaces vercel.json) —
  without a framework set, routes 404. Security headers stay in `next.config.ts`.
- **Env is validated at build** via `src/env.ts` (imported in `next.config.ts`). Import
  `env` from `@/env`, not `process.env`. Bypass with `SKIP_ENV_VALIDATION=1`.
